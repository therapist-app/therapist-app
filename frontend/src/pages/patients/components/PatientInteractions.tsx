...
const transformDataForHeatmap = (data: InteractionData[]): HeatMapData[] => {
  // Create array of hours (0-23) in 2-hour intervals for better performance
  ...
  data.forEach(item => {
    ...
    if (!daysMap.has(shortDate)) {
      daysMap.set(shortDate, Array(12).fill(0))
    }
    const dayData = daysMap.get(shortDate)!
    // Group hours into 2-hour intervals
    const hourIndex = Math.floor(item.hour / 2)
    dayData[hourIndex] += item.value
  })
...
const PatientInteractionsComponent = (): ReactElement => {
  ...
  const filteredData = useMemo(() => 
    interactionType === 'all' 
      ? data 
      : data.filter((d) => d.type === interactionType),
    [data, interactionType]
  )

  // Memoize heatmap data transformation
  const heatmapData = useMemo(() => 
    transformDataForHeatmap(filteredData),
    [filteredData]
  )
