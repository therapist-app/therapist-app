...
export async function getAllChatbotTemplatesForTherapist(therapistId: string): Promise<any[]> {
...
export async function createChatbotTemplate(
  ...
  const response = await api.post(`/api/therapists/${therapistId}/chatbot-templates`, createChatbotTemplateDTO);
  return response.data;
}
...
export async function cloneChatbotTemplate(therapistId: string, chatbotId: string): Promise<ChatbotTemplateOutputDTO> {
  const response = await api.post(`/api/therapists/${therapistId}/chatbot-templates/${chatbotId}/clone`);
  return response.data;
}
