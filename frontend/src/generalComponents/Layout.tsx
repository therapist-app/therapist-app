...
import {
  findPageTrace, getPageFromPath, getPageName, getPathFromPage, PAGES
}                                          from '../utils/routes';

interface LayoutProps { children: ReactNode }
interface LayoutLocationState { patientId?: string }
...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  useEffect(() => { if (isExplicitCtx) sessionStorage.setItem('activePatientId', explicitId!); },
            [isExplicitCtx, explicitId]);

  const forwardPatientId = isExplicitCtx ? explicitId : undefined;

  const currentPage     = getPageFromPath(location.pathname);
  const currentPageName = currentPage === PAGES.CHATBOT_TEMPLATES_DETAILS_PAGE
  ...
  const { stream: typingStream, running: isStreaming } = useTypewriter(
    lastAssistant?.chatRole === ChatMessageDTOChatRoleEnum.Assistant ? lastAssistant.content : undefined
  );
  ...
  const sendMessage = async () => {
    ...
    setWaiting(true);
    try {
      await dispatch(chatWithTherapistChatbot({
        newMessage: msg,
    ...
    <>
      ...
      <Box sx={{ display: 'flex' }}>
        <AppBar position="fixed" sx={{
          zIndex: (t) => t.zIndex.drawer + 1, bgcolor: 'white', color: 'black',
          boxShadow: 'none', borderBottom: '1px solid #e0e0e0',
          width: `calc(100% - ${drawerWidth}px)`, ml: `${drawerWidth}px`
        }}>
          <Toolbar>
            <Typography variant="h1" sx={{ flexGrow: 1 }}>{currentPageName}</Typography>
            <IconButton sx={{ mr: 1 }} onClick={() => navigate(getPathFromPage(PAGES.SETTINGS_PAGE))}>
              <SettingsIcon />
            </IconButton>
            <IconButton onClick={() => { dispatch(logoutTherapist()); navigate(getPathFromPage(PAGES.LOGIN_PAGE)); }}>
        ...
        <Drawer variant="permanent" sx={{
          width: drawerWidth,
          '& .MuiDrawer-paper': { width: drawerWidth, bgcolor: '#121621', color: '#9EA2A8', overflow: 'hidden' }
        }}>
          <img src={logo} alt="UZH Chatbot" style={{ width: '100%', marginTop: '-95px', marginLeft: '-20px' }} />
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, pl: 2 }}>
            {[PAGES.HOME_PAGE, ...pageTrace.slice(1)].map((page, idx) => {
              const pathParams = page === PAGES.PATIENTS_DETAILS_PAGE && forwardPatientId
                ? { patientId: forwardPatientId }
              ...
              return (
                <React.Fragment key={page}>
                  ...
                  <Button
                    ...
                    onClick={() => forwardPatientId && page !== PAGES.HOME_PAGE
                      ? navigate(path, { state: { patientId: forwardPatientId } })
                      : navigate(path)}
                    sx={{
                      color: 'white', bgcolor: active ? '#1F2937' : 'transparent',
                      textTransform: 'none', fontWeight: active ? 600 : 400,
                      fontSize: '18px', justifyContent: 'flex-start', width: '200px', px: 1.5
                    }}
                  >
                    {labelFor(page)}
                  </Button>
        ...
        <Box sx={{
          position: 'fixed', top: 64, left: drawerWidth,
          height: 'calc(100vh - 164px)', bgcolor: '#b4b6b4',
          visibility: expanded ? 'visible' : 'hidden',
          width: `calc(100vw - ${drawerWidth}px)`, display: 'flex', flexDirection: 'column'
        }}>
          <List ref={listRef} sx={{
            overflowY: 'auto', flexGrow: 1, p: '20px 40px 20px 30px',
            display: 'flex', flexDirection: 'column', gap: 1
          }}>
            {messages.map((m, i) => {
              const isLastAssistant = i === messages.length - 1 && m.chatRole === ChatMessageDTOChatRoleEnum.Assistant;
              const body = isLastAssistant && isStreaming
              ...
              return m.chatRole === ChatMessageDTOChatRoleEnum.User ? (
                <ListItem key={i} sx={{ display: 'flex', justifyContent: 'flex-end' }}>
                  <Box sx={{ maxWidth: '80%', ml: 'auto' }}>
                    <Typography variant="caption" sx={{ display: 'block', ml: 1 }}>You</Typography>
                    <Box sx={{ py: 1, px: 2, bgcolor: 'primary.main', color: 'white', borderRadius: '20px' }}>
                ...
                <ListItem key={i} sx={{ alignItems: 'flex-start', flexDirection: 'row' }}>
                  ...
                  <Box sx={{ maxWidth: '80%' }}>
                    <Typography variant="caption" sx={{ display: 'block', ml: 1 }}>Chatbot</Typography>
                    <Paper sx={{ py: 1, px: 2, bgcolor: '#E5E5E5', borderRadius: '20px', display: 'inline-block' }}>
                      <Typography variant="body1">{body}</Typography>
                    </Paper>
                  </Box>
                </ListItem>
            ...
            {waiting && (
              <ListItem sx={{ alignItems: 'flex-start', flexDirection: 'row' }}>
                ...
                <Box sx={{ maxWidth: '80%' }}>
                  <Typography variant="caption" sx={{ display: 'block', ml: 1 }}>Chatbot</Typography>
                  <Box sx={{
                    px: 2, bgcolor: '#E5E5E5', borderRadius: '20px', minHeight: '40px',
        ...
        <Box sx={{
          position: 'fixed', bottom: 0, left: drawerWidth, right: 0,
          bgcolor: '#b4b6b4', display: 'flex', gap: 2, alignItems: 'center', p: 2, zIndex: 1000
        }}>
          <TextField
            ...
            sx={{ bgcolor: 'white', borderRadius: 1, '& .MuiOutlinedInput-root': { height: '60px' } }}
          />
          <IconButton onClick={sendMessage} sx={{ position: 'absolute', right: expanded ? 30 : 70, bottom: 30 }}>
            <SendIcon sx={{ color: 'black' }} />
          </IconButton>
          {!expanded && (
            <IconButton sx={{ color: 'black', height: 30, width: 30 }} onClick={() => setExpanded(true)}>
              <ExpandLessIcon />
            </IconButton>
          )}
        </Box>
        ...
        {expanded && (
          <IconButton
            ...
            onClick={() => { dispatch(clearMessages()); setInput(''); setExpanded(false); }}
          >
            <ExpandMoreIcon />
          </IconButton>
        )}
