...
public class PatientAppContextService {
    ...
    private String latestGad7(String patientId) {
        try {
            ...
            List<PsychologicalTestOutputDTO> results = apiResults.stream()
            ...
            PsychologicalTestOutputDTO latest = results.stream()
                    .max(Comparator.comparing(PsychologicalTestOutputDTO::getCompletedAt))
                    .orElseThrow();

            int totalScore = latest.getQuestions().stream()
                    .mapToInt(q -> {
            ...
            return """
               ...
               """.formatted(
                    latest.getCompletedAt().atZone(ZONE).toLocalDate(),
                    totalScore
            );
            ...
            for (LogTypes type : LogTypes.values()) {
                ...
                if (!counts.isEmpty()) {
                    ...
                    counts.entrySet().stream()
                            ...
                            .forEach(e -> sb.append("W").append(e.getKey())
